MATCH (s:Sequence)
WHERE s.regionCode IN ['PNG', 'KSTR', 'SML', 'BLG']
WITH count(s) AS size
CREATE (task:Task {name: 'BaltoSlavic (518) [RUS_all]', region:'RUS_all', count: size});

// RSRS
MATCH (t:Task {region:'RUS_all'})
...
MATCH (task:Task {region:'RUS_all'})-[:RSRS]->(d:DataNode)
...
MATCH (task:Task {region:'RUS_all'})-[:RSRS]->(rsrs:DataNode {type:'rsrs'})
CREATE (rsrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'RUS_all'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'RUS_all'})-[:RCRS]->(d:DataNode)
...
MATCH (task:Task {region:'RUS_all'})-[:RCRS]->(rcrs:DataNode {type:'rcrs'})
CREATE (rcrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'RUS_all'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
CALL {
    WITH i
		MATCH (seq:Sequence)
		WHERE seq.regionCode IN ['PNG', 'KSTR', 'SML', 'BLG']
...
MATCH (t:Task {region:'RUS_all'})-[:WILDTYPE]->(wt:DataNode)
...
MATCH (task:Task {region:'RUS_all'})-[:WILDTYPE]->(d:DataNode)
...
MATCH (task:Task {region:'RUS_all'})-[:WILDTYPE]->(rcrs:DataNode {type:'wildtype'})
CREATE (rcrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'RUS_all'})-[:WILDTYPE]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
WITH sum(d.percent * d.distance) AS expected_value, rcrs
WITH rcrs.stdev/expected_value AS variation_coef, expected_value, rcrs 
SET rcrs.expected_value = expected_value, rcrs.variation_coef = variation_coef
RETURN rcrs;
...
CALL {
    MATCH (a:Sequence), (b:Sequence)
		WHERE a.id < b.id AND a.regionCode IN ['PNG', 'KSTR', 'SML', 'BLG'] AND b.regionCode IN ['PNG', 'KSTR', 'SML', 'BLG']
		RETURN apoc.text.hammingDistance(a.sequence, b.sequence) AS pair_distances
}

MATCH (task:Task {region:'RUS_all'})-[:PAIR]->(d:DataNode)
...
MATCH (task:Task {region:'RUS_all'})-[:PAIR]->(n:DataNode {type:'pair'})
CREATE (n)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'RUS_all'})-[:PAIR]->(n:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'RUS_all'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rcrs
SET rcrs.amount=amo;

MATCH (task:Task {region:'RUS_all'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rsrs
SET rsrs.amount=amo;

// Haplogroups
MATCH (task:Task {region:'RUS_all'}), (seq:Sequence)
WHERE seq.regionCode IN ['PNG', 'KSTR', 'SML', 'BLG']
...
MATCH (:Task {region:'RUS_all'})-[:RCRS]->(d:DataNode)
...
UNWIND range(0, 376) AS i
WITH apoc.coll.occurrences(collect(any(seq IN seqs WHERE apoc.text.charAt(seq, i) <> apoc.text.charAt(bseq, i))), true) AS popMutations
MATCH (:Task {region:'RUS_all'})-[:RSRS]->(d:DataNode)
SET d.popMutations = popMutations;