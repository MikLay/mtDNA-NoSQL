MATCH (s:Sequence)
WHERE s.regionCode IN ['ZA', 'ST', 'IF']
WITH count(s) AS size
CREATE (task:Task {name: 'Ukraine (142 ланцюжків) [UKR_Karpatska]', region:'UKR_Karpatska', count: size});

// RSRS
MATCH (t:Task {region:'UKR_Karpatska'})
...
MATCH (task:Task {region:'UKR_Karpatska'})-[:RSRS]->(d:DataNode)
...
MATCH (task:Task {region:'UKR_Karpatska'})-[:RSRS]->(rsrs:DataNode {type:'rsrs'})
CREATE (rsrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount});

MATCH (task:Task {region:'UKR_Karpatska'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'UKR_Karpatska'})-[:RCRS]->(d:DataNode)
...
MATCH (task:Task {region:'UKR_Karpatska'})-[:RCRS]->(rcrs:DataNode {type:'rcrs'})
CREATE (rcrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR_Karpatska'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
CALL {
    WITH i
		MATCH (seq:Sequence)
		WHERE seq.regionCode IN ['ZA', 'ST', 'IF']
...
MATCH (t:Task {region:'UKR_Karpatska'})-[:WILDTYPE]->(wt:DataNode)
...
MATCH (task:Task {region:'UKR_Karpatska'})-[:WILDTYPE]->(d:DataNode)
...
MATCH (task:Task {region:'UKR_Karpatska'})-[:WILDTYPE]->(rcrs:DataNode {type:'wildtype'})
CREATE (rcrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR_Karpatska'})-[:WILDTYPE]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
WITH sum(d.percent * d.distance) AS expected_value, rcrs
WITH rcrs.stdev/expected_value AS variation_coef, expected_value, rcrs 
SET rcrs.expected_value = expected_value, rcrs.variation_coef = variation_coef
RETURN rcrs;
...
CALL {
    MATCH (a:Sequence), (b:Sequence)
		WHERE a.id < b.id AND a.regionCode IN ['ZA', 'ST', 'IF'] AND b.regionCode IN ['ZA', 'ST', 'IF']
		RETURN apoc.text.hammingDistance(a.sequence, b.sequence) AS pair_distances
}

MATCH (task:Task {region:'UKR_Karpatska'})-[:PAIR]->(d:DataNode)
...
MATCH (task:Task {region:'UKR_Karpatska'})-[:PAIR]->(n:DataNode {type:'pair'})
CREATE (n)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR_Karpatska'})-[:PAIR]->(n:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'UKR_Karpatska'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rcrs
SET rcrs.amount=amo;

MATCH (task:Task {region:'UKR_Karpatska'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rsrs
SET rsrs.amount=amo;

// Haplogroups
MATCH (task:Task {region:'UKR_Karpatska'}), (seq:Sequence)
WHERE seq.regionCode IN ['ZA', 'ST', 'IF']
...
MATCH (:Task {region:'UKR_Karpatska'})-[:RCRS]->(d:DataNode)
...
UNWIND range(0, 376) AS i
WITH apoc.coll.occurrences(collect(any(seq IN seqs WHERE apoc.text.charAt(seq, i) <> apoc.text.charAt(bseq, i))), true) AS popMutations
MATCH (:Task {region:'UKR_Karpatska'})-[:RSRS]->(d:DataNode)
SET d.popMutations = popMutations;