MATCH (s:Sequence)
WHERE s.regionCode IN ['SML', 'BLG', 'BRST', 'GML', 'VTB'] OR s.countryCode = 'UKR'
WITH count(s) AS size
CREATE (task:Task {name: 'BaltoSlavic, Ukraine  [UKR_BEL_RUS_south]', region:'UKR_BEL_RUS_south', count: size});

// RSRS
MATCH (t:Task {region:'UKR_BEL_RUS_south'})
...
MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:RSRS]->(d:DataNode)
...
MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:RSRS]->(rsrs:DataNode {type:'rsrs'})
CREATE (rsrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:RCRS]->(d:DataNode)
...
MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:RCRS]->(rcrs:DataNode {type:'rcrs'})
CREATE (rcrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
CALL {
    WITH i
		MATCH (seq:Sequence)
		WHERE seq.regionCode IN ['SML', 'BLG', 'BRST', 'GML', 'VTB'] OR seq.countryCode = 'UKR'
...
MATCH (t:Task {region:'UKR_BEL_RUS_south'})-[:WILDTYPE]->(wt:DataNode)
...
MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:WILDTYPE]->(d:DataNode)
...
MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:WILDTYPE]->(rcrs:DataNode {type:'wildtype'})
CREATE (rcrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:WILDTYPE]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
WITH sum(d.percent * d.distance) AS expected_value, rcrs
WITH rcrs.stdev/expected_value AS variation_coef, expected_value, rcrs 
SET rcrs.expected_value = expected_value, rcrs.variation_coef = variation_coef
RETURN rcrs;
...
CALL {
    MATCH (a:Sequence), (b:Sequence)
		WHERE a.id < b.id AND (a.regionCode IN ['SML', 'BLG', 'BRST', 'GML', 'VTB'] OR a.countryCode = 'UKR') AND (b.regionCode IN ['SML', 'BLG', 'BRST', 'GML', 'VTB'] OR b.countryCode = 'UKR')
		RETURN apoc.text.hammingDistance(a.sequence, b.sequence) AS pair_distances
}

MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:PAIR]->(d:DataNode)
...
MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:PAIR]->(n:DataNode {type:'pair'})
CREATE (n)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:PAIR]->(n:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rcrs
SET rcrs.amount=amo;

MATCH (task:Task {region:'UKR_BEL_RUS_south'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rsrs
SET rsrs.amount=amo;

// Haplogroups
MATCH (task:Task {region:'UKR_BEL_RUS_south'}), (seq:Sequence)
WHERE seq.regionCode IN ['SML', 'BLG', 'BRST', 'GML', 'VTB'] OR seq.countryCode = 'UKR'
...
MATCH (:Task {region:'UKR_BEL_RUS_south'})-[:RCRS]->(d:DataNode)
...
UNWIND range(0, 376) AS i
WITH apoc.coll.occurrences(collect(any(seq IN seqs WHERE apoc.text.charAt(seq, i) <> apoc.text.charAt(bseq, i))), true) AS popMutations
MATCH (:Task {region:'UKR_BEL_RUS_south'})-[:RSRS]->(d:DataNode)
SET d.popMutations = popMutations;