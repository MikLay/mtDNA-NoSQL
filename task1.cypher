MATCH (s:Sequence {countryCode:'UKR'})
WITH count(s) AS size
CREATE (task:Task {name: 'Ukraine (607 ланцюжків) [UKR]', region:'UKR', count: size});


// RSRS
MATCH (t:Task {region:'UKR'})
...
MATCH (task:Task {region:'UKR'})-[:RSRS]->(d:DataNode)
...
MATCH (task:Task {region:'UKR'})-[:RSRS]->(rsrs:DataNode {type:'rsrs'})
CREATE (rsrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
CALL {
    ...
    RETURN apoc.text.hammingDistance(s.sequence, brs.sequence) AS rсrs_distances
}

MATCH (task:Task {region:'UKR'})-[:RCRS]->(d:DataNode)

WITH collect(rсrs_distances) AS distances, d
// MAX, MIN, STDEV
WITH apoc.coll.max(distances) AS max, apoc.coll.min(distances) AS min, apoc.coll.stdev(distances) AS stdev, distances,d 
SET d.max = max, d.min=min, d.stdev = stdev
...
MATCH (task:Task {region:'UKR'})-[:RCRS]->(rсrs:DataNode {type:'rcrs'})
CREATE (rсrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR'})-[:RCRS]->(rсrs:DataNode)<-[:HAS]-(d:DistanceNode)
WITH sum(d.percent * d.distance) AS expected_value, rсrs
WITH rсrs.stdev/expected_value AS variation_coef, expected_value, rсrs 
SET rсrs.expected_value = expected_value, rсrs.variation_coef = variation_coef
RETURN rсrs;

CALL {
	MATCH (task:Task {region:'UKR'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
CALL {
    WITH i
    MATCH (seq:Sequence {countryCode:'UKR'})
...
MATCH (t:Task {region:'UKR'})-[:WILDTYPE]->(wt:DataNode)
...
MATCH (task:Task {region:'UKR'})-[:WILDTYPE]->(d:DataNode)
...
MATCH (task:Task {region:'UKR'})-[:WILDTYPE]->(rсrs:DataNode {type:'wildtype'})
CREATE (rсrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR'})-[:WILDTYPE]->(rсrs:DataNode)<-[:HAS]-(d:DistanceNode)
WITH sum(d.percent * d.distance) AS expected_value, rсrs
WITH rсrs.stdev/expected_value AS variation_coef, expected_value, rсrs 
SET rсrs.expected_value = expected_value, rсrs.variation_coef = variation_coef
RETURN rсrs;
...
CALL {
	MATCH (task:Task {region:'UKR'})-[:WILDTYPE]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
	RETURN d.distance AS moda, rcrs
	ORDER BY d.amount DESC
	LIMIT 1
}
...
CALL {
    MATCH (a:Sequence {countryCode:'UKR'}), (b:Sequence {countryCode:'UKR'})
	WHERE a.id < b.id
	RETURN apoc.text.hammingDistance(a.sequence, b.sequence) AS pair_distances
}

MATCH (task:Task {region:'UKR'})-[:PAIR]->(d:DataNode)
...
MATCH (task:Task {region:'UKR'})-[:PAIR]->(n:DataNode {type:'pair'})
CREATE (n)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'UKR'})-[:PAIR]->(n:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'UKR'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rcrs
SET rcrs.amount=amo;

MATCH (task:Task {region:'UKR'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rsrs
SET rsrs.amount=amo;

// Haplogroups
MATCH (task:Task {region:'UKR'}), (seq:Sequence {countryCode:'UKR'})
...
MATCH (bs:BasicSequence {id:'rCRS'}), (s:Sequence {countryCode:'UKR'})
...
MATCH (:Task {region:'UKR'})-[:RCRS]->(d:DataNode)
SET d.popMutations = popMutations;

MATCH (bs:BasicSequence {id:'RSRS'}), (s:Sequence {countryCode:'UKR'})
WITH collect(bs.sequence)[0] AS bseq, collect(s.sequence) as seqs
UNWIND range(0, 376) AS i
WITH apoc.coll.occurrences(collect(any(seq IN seqs WHERE apoc.text.charAt(seq, i) <> apoc.text.charAt(bseq, i))), true) AS popMutations
MATCH (:Task {region:'UKR'})-[:RSRS]->(d:DataNode)
SET d.popMutations = popMutations;