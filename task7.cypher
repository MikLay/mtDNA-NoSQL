MATCH (s:Sequence)
WHERE s.regionCode IN ['PNG', 'KSTR']
WITH count(s) AS size
CREATE (task:Task {name: 'BaltoSlavic (223) [RUS_north]', region:'RUS_north', count: size});

// RSRS
MATCH (t:Task {region:'RUS_north'})
...
MATCH (task:Task {region:'RUS_north'})-[:RSRS]->(d:DataNode)
...
MATCH (task:Task {region:'RUS_north'})-[:RSRS]->(rsrs:DataNode {type:'rsrs'})
CREATE (rsrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'RUS_north'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'RUS_north'})-[:RCRS]->(d:DataNode)
...
MATCH (task:Task {region:'RUS_north'})-[:RCRS]->(rcrs:DataNode {type:'rcrs'})
CREATE (rcrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'RUS_north'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
...
CALL {
    WITH i
		MATCH (seq:Sequence)
		WHERE seq.regionCode IN ['PNG', 'KSTR']
...
MATCH (t:Task {region:'RUS_north'})-[:WILDTYPE]->(wt:DataNode)
...
MATCH (task:Task {region:'RUS_north'})-[:WILDTYPE]->(d:DataNode)
...
MATCH (task:Task {region:'RUS_north'})-[:WILDTYPE]->(rcrs:DataNode {type:'wildtype'})
CREATE (rcrs)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'RUS_north'})-[:WILDTYPE]->(rcrs:DataNode)<-[:HAS]-(d:DistanceNode)
WITH sum(d.percent * d.distance) AS expected_value, rcrs
WITH rcrs.stdev/expected_value AS variation_coef, expected_value, rcrs 
SET rcrs.expected_value = expected_value, rcrs.variation_coef = variation_coef
RETURN rcrs;
...
CALL {
    MATCH (a:Sequence), (b:Sequence)
		WHERE a.id < b.id AND a.regionCode IN ['PNG', 'KSTR'] AND b.regionCode IN ['PNG', 'KSTR']
		RETURN apoc.text.hammingDistance(a.sequence, b.sequence) AS pair_distances
}

MATCH (task:Task {region:'RUS_north'})-[:PAIR]->(d:DataNode)
...
MATCH (task:Task {region:'RUS_north'})-[:PAIR]->(n:DataNode {type:'pair'})
CREATE (n)<-[:HAS]-(d:DistanceNode {distance:distance, percent:percent, amount:amount}); 

MATCH (task:Task {region:'RUS_north'})-[:PAIR]->(n:DataNode)<-[:HAS]-(d:DistanceNode)
...
MATCH (task:Task {region:'RUS_north'})-[:RCRS]->(rcrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rcrs
SET rcrs.amount=amo;

MATCH (task:Task {region:'RUS_north'})-[:RSRS]->(rsrs:DataNode)<-[:HAS]-(d1:DistanceNode)
WITH sum(d1.distance) AS amo, rsrs
SET rsrs.amount=amo;

// Haplogroups
MATCH (task:Task {region:'RUS_north'}), (seq:Sequence)
WHERE seq.regionCode IN ['PNG', 'KSTR']
...
MATCH (:Task {region:'RUS_north'})-[:RCRS]->(d:DataNode)
...
UNWIND range(0, 376) AS i
WITH apoc.coll.occurrences(collect(any(seq IN seqs WHERE apoc.text.charAt(seq, i) <> apoc.text.charAt(bseq, i))), true) AS popMutations
MATCH (:Task {region:'RUS_north'})-[:RSRS]->(d:DataNode)
SET d.popMutations = popMutations;